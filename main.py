...
@app.route('/auto')
def auto():
    return render_template('/auto.html', title='авторизация', menu=menu)
    ...
    if request.method == 'POST':
        if (len(request.form['search']) > 5):
            ...
            if res:
                ...
                flash('такой псевдоним уже существует')
        else:
...
def redirect_url(link_name):
    lnk = dbase.searchLinnk(link_name)
    print(lnk)
    print(lnk[0])
    if lnk is None:
        ...
        if type == 'private':
            if session.get('user'):
                ...
                flash('сначала войдите или зарегистрируйтесь')
...
def red():
    if request.method == 'POST':
        ...
        red = linki[2].split('/')
        # print(linki)
        return render_template('red.html', linkk=linki, menu=menuauth, redd=red)
    else:
        return 'dct plolo'
